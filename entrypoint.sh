...
export HTUSERNAME=$(curl -X GET -H "Authorization: Bearer $IAM_TOKEN" https://payload.lockbox.api.cloud.yandex.net/lockbox/v1/secrets/$SECRET_ID/payload -s | jq '(.entries[] | select( .key == "kafka-ui-user")).textValue' -r)
export HTPASSWORD=$(curl -X GET -H "Authorization: Bearer $IAM_TOKEN" https://payload.lockbox.api.cloud.yandex.net/lockbox/v1/secrets/$SECRET_ID/payload -s | jq '(.entries[] | select( .key == "kafka-ui-password")).textValue' -r)
...
# using certbot instead of self-signed certificate
